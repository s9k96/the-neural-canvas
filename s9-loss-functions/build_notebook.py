"""Bake s9_loss_harness.py into an executed .ipynb, then bake the results into the widget.

The repo's generate-then-bake pattern (S4, S6, S7), applied to a notebook: the `.py` is the
source of truth, the `.ipynb` is a build artifact that carries its outputs so a reviewer sees
the numbers without running anything, and `loss-harness.html` gets the same numbers as a
literal JS blob.

    python build_notebook.py            # convert, execute, bake
...
SRC = HERE / "s9_loss_harness.py"
NB = HERE / "s9_loss_harness.ipynb"
EVIDENCE = HERE / "out" / "evidence.json"
WIDGET = HERE / "loss-harness.html"

BEGIN = "/* BEGIN GENERATED s9data */"
END = "/* END GENERATED s9data */"
...
def install_cell():
    ...
    return {
        "cell_type": "code", "id": "s9setup", "metadata": {}, "execution_count": None, "outputs": [],
...
def corpus_cell():
    ...
    return {
        "cell_type": "code", "id": "s9corpus", "metadata": {}, "execution_count": None, "outputs": [],
        "source": ("# The harness reads real documents from s6-dataset-creation/corpus/, which lives in\n"
                   "# this repo. On Colab, clone it; locally this is already on disk and does nothing.\n"
                   "import os\n"
                   "if not os.path.isdir('s6-dataset-creation') and not os.path.isdir('../s6-dataset-creation'):\n"
                   "    !git clone -q https://github.com/s9k96/the-neural-canvas.git\n"
                   "    os.chdir('the-neural-canvas/s9-loss-functions')\n"),
    }
...
def bake_widget():
    """Inject evidence.json into loss-harness.html between the generated markers."""
    if not (WIDGET.exists() and EVIDENCE.exists()):
        return False
    ev = json.loads(EVIDENCE.read_text(encoding="utf-8"))
    blob = f"{BEGIN}\nconst S9DATA = {json.dumps(ev, indent=2, ensure_ascii=False)};\n{END}"
...
def main():
    ...
    print("executing (this runs the whole harness; several minutes on CPU)…")
    failed = execute(NB)
    if failed:
