...
```bash
.venv/Scripts/python s06-dataset-creation/run_demo.py            # the complete demonstration
.venv/Scripts/python s06-dataset-creation/tests/test_invariants.py   # 13 invariants
```
...
```bash
.venv/Scripts/python s06-dataset-creation/prepare_corpus.py
```
...
| `out/s6data.js` | **yes** (116 KB) | Page data, so `dataset-creation.html` renders from a clone. Same convention as `s04-data-cleaning/out/`. |
...
```bash
.venv/Scripts/python -m tds.evidence submission_artifacts    # from s06-dataset-creation/
```
