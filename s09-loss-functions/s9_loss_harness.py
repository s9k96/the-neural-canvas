...
# **Sarvam-1, 68,096 tokens**, sha256 `bb5115a3…`, the same hash `s06-dataset-creation/tds/shards.py`
# pins and `s07-model-internals` measured against. The text is S6's committed `corpus/`, so the
...
if HERE.name != "s09-loss-functions" and (HERE / "s09-loss-functions").is_dir():
    HERE = HERE / "s09-loss-functions"                     # notebook launched from the repo root
ROOT = HERE.parent
OUT = HERE / "out"
