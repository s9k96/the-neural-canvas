"""Bake s10_training_loop.py into an executed .ipynb, then bake the results into the widget.
...
the numbers without running anything, and `training-step.html` gets the same numbers as a
literal JS blob.
...
SRC = HERE / "s10_training_loop.py"
NB = HERE / "s10_training_loop.ipynb"
EVIDENCE = HERE / "out" / "evidence.json"
WIDGET = HERE / "training-step.html"

BEGIN = "/* BEGIN GENERATED s10data */"
END = "/* END GENERATED s10data */"
...
def install_cell():
    ...
    return {
        "cell_type": "code", "id": "s10setup", "metadata": {}, "execution_count": None, "outputs": [],
...
def corpus_cell():
    ...
    return {
        "cell_type": "code", "id": "s10corpus", "metadata": {}, "execution_count": None, "outputs": [],
        "source": ("# The harness reads real documents from s06-dataset-creation/corpus/, which lives in\n"
                   ...
                   "    os.chdir('the-neural-canvas/s10-training-loop')\n"),
    }
...
def bake_widget():
    """Inject evidence.json into training-step.html between the generated markers."""
    if not (WIDGET.exists() and EVIDENCE.exists()):
        return False
    ev = json.loads(EVIDENCE.read_text(encoding="utf-8"))
    blob = f"{BEGIN}\nconst S10DATA = {json.dumps(ev, indent=2, ensure_ascii=False)};\n{END}"
