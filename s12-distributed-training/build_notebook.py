"""Bake s12_distributed.py into an executed .ipynb, then bake the results into the widget.
...
sees the numbers without running anything, and `the-redundancy-tax.html` gets the same
numbers as a literal JS blob.

    python build_notebook.py              # convert, execute, bake   (~10 min, CPU)
...
SRC = HERE / "s12_distributed.py"
NB = HERE / "s12_distributed.ipynb"
EVIDENCE = HERE / "out" / "evidence.json"
WIDGET = HERE / "the-redundancy-tax.html"

BEGIN = "/* BEGIN GENERATED s12data */"
END = "/* END GENERATED s12data */"
...
def install_cell():
    ...
    return {
        "cell_type": "code", "id": "s12setup", "metadata": {}, "execution_count": None, "outputs": [],
...
def corpus_cell():
    ...
    return {
        "cell_type": "code", "id": "s12corpus", "metadata": {}, "execution_count": None, "outputs": [],
        "source": ("# The harness reads real documents from s06-dataset-creation/corpus/, which lives in\n"
                   ...
                   "    os.chdir('the-neural-canvas/s12-distributed-training')\n"),
    }
...
def bake_widget():
    """Inject evidence.json into the-redundancy-tax.html between the generated markers."""
    if not (WIDGET.exists() and EVIDENCE.exists()):
        return False
    ev = json.loads(EVIDENCE.read_text(encoding="utf-8"))
    blob = f"{BEGIN}\nconst S12DATA = {json.dumps(ev, indent=2, ensure_ascii=False)};\n{END}"
...
def main():
    ...
    print("executing (this runs the whole harness end to end; ~10 minutes on CPU)…")
    failed = execute(NB)
    if failed:
